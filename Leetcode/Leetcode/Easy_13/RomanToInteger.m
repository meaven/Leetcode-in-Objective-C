...
- (NSInteger)romanToInteger:(NSString *)romanString {
    ...
    NSString *key = [NSString stringWithFormat:@"%C", [romanString characterAtIndex:[romanString length] - 1]];
    
    NSInteger result = [mappingDictionary[key] integerValue];
    NSInteger previous = result;
    for (NSInteger i = [romanString length] - 2; i >= 0; i--) {
        key = [NSString stringWithFormat:@"%C", [romanString characterAtIndex:i]];
        NSInteger current = [mappingDictionary[key] integerValue];
        if (current < previous) {
            result -= current;
        } else {
            result += current;
